    ...
    return (
      <Paper className={classes.root}>
        ...
        <div className={classes.tableWrapper}>
          ...
          {products.length > 0 && <Table className={classes.table} aria-labelledby="tableTitle">
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {products.map(product =>
                <ProductsListRow key={product.name} product={product} />
              )}
            </TableBody>
          </Table>}
        </div>
...
const mapStateToProps = state => {
  return {
    isGetting: selectors.productsIsGetting(state) ||
